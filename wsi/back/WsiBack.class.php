...
class WsiBack {
	...
	public function enqueue_wsi_back_styles() {
		...
		wp_register_style('overlay-basic',   WsiCommons::getURL().'/style/jqueryTools/overlay-basic.css'); /*Style pour la box de feedback*/
	...
	public function enqueue_wsi_back_scripts() {
		if (isset($_GET['page']) && $_GET['page'] == 'wp_splash_image') {
			...
			// JQuery Tools
			wp_register_script('jquery.tools.back', WsiCommons::getURL().'/js/jQueryTools/jquery.tools.min.wp-back.v'.JQUERY_TOOLS_FILES_VERSION.'.js'); /*[overlay, overlay.apple, dateinput, validator, toolbox.expose]*/
	...
	<div class="wsi-back wrap">
		...
		<div id="display_feedback">
			<img id="feedback_img" class="tooltipped" data-position="bottom" rel="#feedback"
                 data-tooltip="<?php echo __('Feedback','wp-splash-image'); ?>"
                 alt="<?php echo __('Feedback','wp-splash-image'); ?>"
                 src="<?php echo WsiCommons::getURL(); ?>/style/feedback_logo.png" />
		</div>
